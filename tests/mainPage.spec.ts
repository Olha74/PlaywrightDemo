...
test('Проверка атрибута href элементов навигации- Header', async ({ page }) => {
  await page.goto('https://playwright.dev/');
...
test('Проверка переключения licht mode- Header', async ({ page }) => {
  await page.goto('https://playwright.dev/');
  await page.getByRole('button', { name: 'Switch between dark and light' }).click();
  // await expect (page.locator('div').toHaveatr
});
